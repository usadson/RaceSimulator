...
RenderWindow window = new(new VideoMode(1280, 720), "Carro di Mario");
...
int scalingIndex = scalings.Select((f, index) => (f, index)).First(p => p.f == 0.1f).index;
...
Dictionary<SectionTypes, Sprite> sectionSprites = new()
{
...
Dictionary<SectionTypes, IntRect> sectionSizeOverrides = new()
{
...
float DrawPlayerIcon(Character character, float x, float y)
{
...
void DrawTrack(SectionTypes? specific)
{
